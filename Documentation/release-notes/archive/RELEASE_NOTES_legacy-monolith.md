...
v7.6.5br2 focuses on operational safety, persistence reliability, and spectrum runtime stability while preserving ApeX all-in-one feature coverage.

### Compatibility

- Supported hardware: UV-K1 and UV-K5 V3 (PY32F071).
- Not intended for UV-K5 V1 or non-PY32F071 variants.

### Key Improvements

1. Low-voltage runtime protection
- TX is blocked below the calibrated 7.0V minimum safety threshold.
- EEPROM/flash writes are blocked below the calibrated 7.0V minimum safety threshold.
- Pending saves are deferred and retried when voltage is safe.

2. Persistence integrity hardening
- Spectrum settings persistence uses native full-block flash writes (replacing legacy 8-byte compatibility behavior for this block).
- Transactional A/B settings snapshots added with:
  - versioned header,
  - checksum validation,
  - generation-based boot selection,
  - write-readback verification with retry/fallback.

3. Spectrum reliability and control consistency
- Improved sweep-data integrity and RSSI handling safety.
- Preserved immediate control behavior for modulation/bandwidth updates and stable restore behavior across reboot.

4. ApeX workflow alignment
- NOAA quick access and function-key behavior aligned for field use.
- Scan/Spectrum key-path conflicts removed in current ApeX mapping.

### Why This Matters

- Reduces brownout-induced instability on weak or aging battery packs.
- Improves resilience against interrupted or low-power persistence failures.
- Improves confidence that last valid settings are recoverable after abnormal shutdown.

### Validation Snapshot

- Build path: `./compile-with-docker.sh ApeX`
- Build result: successful
- Latest observed memory summary:
...
- After flashing, verify battery calibration and test TX at normal battery state.
- If battery is weak, expect deferred saves and TX blocking until safe voltage is restored.

---

## Legacy Detailed Notes (Historical)
...
### April 2026 — Low-Voltage Safety & Persistence Hardening

This update hardens runtime safety for weak-battery operation and reduces persistence corruption risk on UV-K1 / UV-K5 V3 targets.

- Added calibrated 7.0V minimum safety threshold for TX. Transmit is now blocked before the radio enters an unstable brownout-prone condition.
- Added calibrated 7.0V minimum safety threshold for EEPROM/flash persistence. Settings writes are blocked when battery voltage is below the safe threshold.
- Deferred pending settings, VFO, FM, and channel-save requests until voltage becomes safe again, instead of dropping them.
- Hardened spectrum settings persistence by switching the 20-byte settings block to the native flash writer path, avoiding partial writes through the legacy 8-byte compatibility writer.
- Added transactional settings persistence snapshots (A/B) with versioned header, payload checksum validation, and generation-based boot recovery.
- Added snapshot write-readback verification with retry/fallback path to improve commit reliability under transient flash-write faults.
- Kept startup behavior intact by allowing early-boot persistence only before averaged battery telemetry is available.

Result: stronger protection for brand-new and aging radios during weak-battery conditions, reduced risk of save corruption, and more predictable recovery of user changes once power is stable again.
