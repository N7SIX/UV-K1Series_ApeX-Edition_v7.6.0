...
Audit basis:
- Commit window audited: April 1-28, 2026.
...
4. Added VSWR module files and integration scaffolding (`vswr.c`, `vswr_delay.c`, `vswr_delay.h`) in late-April v7.6.6 cycle (b91ae02).
5. Added finalized v7.6.6 build identity in presets (`VERSION_STRING_2=v7.6.6`, ApeX target naming) (090c6cf).

### Improved (Verified April 2026)
1. Improved SysInf menu placement/label behavior for N7SIX feature layout (`SysInf` in menu flow) (fd25a36).
2. Improved hidden-menu entry workflow under F-lock boot path, including hidden-section cursor placement and lock handling (fd25a36).
3. Improved UI presentation with antenna icon positioning and resized display elements in early April UX passes (96d6bf1, bc07378).
