# MDC-1200 Menu & Configuration Guide — post-v7.6.10C

**Date**: 2026-08-16 (original) / 2026-09-06 (interop remap)  
...
The dispatch was remapped to the **Motorola MDC-1200 standard opcode map**, and the receiving radio now plays a **local MDC preamble warble** through its speaker whenever it decodes a valid MDC ID — on simplex and through repeaters.
...
5. **Motorola interop remap** (post-v7.6.10C):
    - Dispatch remapped to Motorola standard opcode map (0x01 PTT ID, 0x30/0x31 Call Alert, etc.)
    - Local MDC preamble warble (`AUDIO_PlayMDCWarble`) for decoded IDs
...
| `MDC-1200` | 5            | Send standard **26-byte** MDC-1200 PTT-ID burst at **end of TX** (EOT-only) |
| `MDC-1200L`| 6            | Send **46-byte** long-burst MDC-1200L PTT-ID at **end of TX** (EOT-only) |
...
**RX behavior:** when the receiving radio decodes a valid MDC ID, it plays a
**local MDC preamble warble** through its speaker and shows the center-line
alert with the Unit ID in **decimal** (Motorola convention). This works on
simplex and through repeaters, independent of whether the repeater relays the
on-air burst audio. Emergency opcodes (`0x81/0x82`) are transmitted **3×
back-to-back** for reliability, and the RX side alerts on Motorola emergency
frames as well as legacy emergency opcodes.
...
handle both lengths. The RF driver (`BK4819_TransmitMDC1200Frame`) loads the
full frame (preamble included) into the TX FIFO via `MDC1200_BuildFifoWords()`,
programs `REG_5D` (`0x1A` = 26, `0x2E` = 46 TX FIFO bytes) and selects TX
timing from the frame length automatically (280 ms / 400 ms).

> Note: a preamble-stripped FIFO variant (`REG_5D` 0x14/0x28) was tried and
> reverted — on air it produced no audible/decodable burst. Do not reintroduce
> it.
