...
**Date**: 2026-08-17 (design) / 2026-08-20 (implementation) / 2026-09-06 (interop remap)  
...
The dispatch was remapped to the **Motorola MDC-1200 standard opcode map**, and the audio alert was changed from a suppressed `AUDIO_PlayBeep()` call to a **local MDC preamble warble** (`AUDIO_PlayMDCWarble()`) that plays through the receiving radio's speaker whenever a valid MDC ID is decoded — on simplex and through repeaters.
...
5. Decode & Validate (App/mdc1200.c)
   MDC1200_DiffDecodeFrame() — XOR differential decode
...
6. Phase 2 Dispatch (App/mdc_handler.c)
   MDC_DispatchFrame(op, arg, unit_id, valid)
   → Duplicate suppression check (identical unit+op within 10 s → silent; emergencies exempt)
    → Update g_MDC_LastRxFrame global state
   → switch(opcode) → Motorola handler
   → Handler sets center_line = CENTER_LINE_MDC_ALERT
   → Handler plays local MDC preamble warble (AUDIO_PlayMDCWarble)
   → gUpdateDisplay = true

7. UI Refresh
   → UI_DisplayMain() → UI_DisplayMDCAlert()
   → Renders opcode name, unit ID (decimal), argument to center line
...
/* Phase 2: Dispatch to opcode handler (Motorola standard map) */
MDC_DispatchFrame(op, arg, unit_id, valid);
```

The dispatch was remapped to the **Motorola MDC-1200 standard opcode map** (0x01 PTT ID,
0x30/0x31 Call Alert, 0x40/0x41 Radio Check, 0x46/0x47 Status, 0x81/0x82 Emergency,
0x83 Emergency Clear). Legacy local opcodes (0x00–0x07) remain dispatched for backward
compatibility. The audio alert was changed from a suppressed `AUDIO_PlayBeep()` call to a
**local MDC preamble warble** (`AUDIO_PlayMDCWarble()`) that plays through the receiving
radio's speaker whenever a valid MDC ID is decoded — on simplex and through repeaters.

### Resulting Handler Actions

1. **PTT ID (0x01)**:
   - Dispatch to MDC_Handle_PttID()
   - Handler: set center_line, play single warble, set 3s timeout

2. **Emergency (0x81/0x82)**:
   - Dispatch to MDC_Handle_Emergency()
   - Handler: modal popup, double warble, manual dismiss

3. **Legacy (0x00/0x02/0x04)**:
   - Dispatch to MDC_Handle_Legacy()
   - Handler: set center_line, play single warble, set 3s timeout
...
| Motorola opcode remap (interop) | ✅ Done |
| Local MDC preamble warble (AUDIO_PlayMDCWarble) | ✅ Done |
| Decimal Unit ID display | ✅ Done |
| RX duplicate suppression (10 s window) | ✅ Done |
...
✅ Phase 2: Opcode dispatch implemented (Motorola standard map)
✅ Phase 2: All Motorola handlers + legacy fallback implemented
✅ Phase 2: Local MDC preamble warble integrated
...
### Integration Tests
- ✅ Opcode dispatch on RX (Motorola + legacy opcodes)
- ✅ Handler invocation for each opcode
- ✅ UI display update (decimal Unit ID)
- ✅ Local MDC preamble warble triggering
...
- ✅ Decode & CRC validation (with XOR differential decode)
- ✅ EEPROM storage fields
- ✅ Display update triggers
- ✅ Opcode-specific handlers (Motorola standard map + legacy fallback)
- ✅ Local MDC preamble warble (AUDIO_PlayMDCWarble)
- ✅ Center-line UI display (decimal Unit ID)
- ✅ Emergency modal with manual dismiss
- ✅ 500ms timeout cleanup
- ✅ RX duplicate suppression (10 s window, emergencies exempt)

Ready for production deployment on post-v7.6.10C firmware.
