...
**Version**: v7.6.10B  
**Build**: n7six.ApeX-k1.v7.6.10B.bin (115,868 B flash, 14,240 B RAM)
...
MDC_Display_State_t g_MDC_DisplayState = {
    .previous_mode = CENTER_LINE_NONE,
    .dismiss_time = 0,
    .is_emergency = false
};
```
...
|-------|---------|------|-------------|
| MDC_UnitID | 0x00A050–51 | 2 B | Destination Unit ID (0x0000–0xFFFF) |
| MDC_DefaultOp | 0x00A052 | 1 B | Opcode (0x00–0x07) |
| MDC_DefaultArg | 0x00A053 | 1 B | Argument (0x00–0x0F) |
| Alias Table | 0x00A158 | 64 B | 8 entries × 8 bytes (unit_id + name) |
...
| Create `MDC_Display_State_t` struct | ✅ Done |
...
| Add menu item for last RX frame | ✅ Done |
| PTT-ID auto-transmit | ✅ Done |
| Contacts alias table | ✅ Done |
...
Ready for production deployment on v7.6.10B firmware.
