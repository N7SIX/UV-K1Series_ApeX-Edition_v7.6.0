...
void CW_TimeSlice10ms(void)
{
    ...
    // CW_COMPOSING: poll the iambic/bug/ultimatic keyer so a paddle press can
    // start dedicated TX. CW_AppUpdate() -> CW_HandleState() reads the paddles
    // and returns CW_ACTION_CARRIER_ON on a key-down; the RF path in CW_AppUpdate
    // then sets gCW_State = CW_SENDING so subsequent ticks dispatch to the keyer
    // via the CW_SENDING branch above instead of falling through to the decoder.
    #ifdef ENABLE_FEAT_N7SIX_CW
    if (!gCW_PlaybackActive)
        CW_AppUpdate();
    #endif

    // If CW_AppUpdate() just transitioned the keyer into TX this tick
    // (it sets gCW_State = CW_SENDING on CW_ACTION_CARRIER_ON), bail out so
    // the RX decoder does not read the TX carrier's RSSI. Subsequent ticks
    // are dispatched by the CW_SENDING branch at the top of this function.
    if (gCW_State == CW_SENDING)
        return;
