...
// Owned here: discriminates the typed-message TX path (CW_TxStateMachine)
// from the (now removed) keyer TX path.
static bool      gCW_PlaybackActive = false;
extern volatile uint32_t gGlobalSysTickCounter;

// PTT press tracker for typed-message TX.
// The hardware PTT is handled outside the keypad matrix (CheckKeys() in
// app.c), so it never produces KEY_PTT bKeyHeld events. CW therefore
// reconstructs short/long press timing from the press/release callback pair
// using gGlobalSysTickCounter (10ms units, same clock used by CW timing).
static uint32_t gCW_PttPressTick = 0;            // systick value at PTT press
static bool     gCW_PttPressedInCompose = false; // press began in CW_COMPOSING
...
static uint32_t  gCW_TxEndTime = 0;  // absolute 1ms end time for current element
...
// ---- UI helpers ----
// TX snapshot: the message is copied at send time so that user edits to
// gCW_Message/gCW_CursorPos during transmission cannot corrupt the output.
// (Declared here, before CW_DrawWrappedTxText, which renders the snapshot
// while a message is being transmitted.)
static char          gCW_TxSnapshot[CW_MSG_MAX_LEN + 1];
static uint8_t       gCW_TxSnapshotLen = 0;

static void CW_DrawWrappedTxText(void)
{
    // During typed-message playback render the snapshot actually being
    // transmitted rather than the live-editable gCW_Message, so the display
    // always agrees with the on-air text.
    const char *text = gCW_PlaybackActive ? gCW_TxSnapshot : gCW_Message;
...
void CW_Render(void)
{
    ...
    switch (gCW_State)
    {
        case CW_COMPOSING:
        {
            ...
            const bool hasTypedText = (gCW_Message[0] != '\0');

            // If user has typed anything, show their message on TX lines
            // instead of the RX decode text. The signal graph on DECODE line
            // still shows reception activity.
            if (hasTypedText)
            {
                CW_DrawWrappedTxText();
            }
            else if (haveDecodedText || CW_Decoder_GetCurrentMorse()[0] != '\0')
            {
...
void CW_Init(void)
{
    ...
    gCW_CursorPos = 0;
    gCW_Message[0] = '\0';
    gCW_WPM = gEeprom.CW_KEY_WPM;
    if (gCW_WPM < 5 || gCW_WPM > 100) {
...
void CW_TimeSlice10ms(void)
{
    ...
    // Drive TX state machine during send (typed-message TX).
...
#ifdef ENABLE_AM_FIX
    rssi_dBm += (gSetting_AM_fix && gRxVfo->Modulation == MODULATION_AM) ? AM_fix_get_gain_diff() : 0;
#endif
#ifdef ENABLE_FEAT_N7SIX
    rssi_dBm += dBmCorrTable[gRxVfo->Band];
#endif
    ...
    if (gCW_KeyTick > 0)
    {
        ...
        // CW_KEY_TICK_RESET_MS is in ms, but we tick at 10ms intervals.
        // Convert to ticks: 800ms / 10ms = 80 ticks.
        if (gCW_KeyTick >= (CW_KEY_TICK_RESET_MS / 10))
        {
...
static void CW_EndDedicatedTx(void)
{
    ...
    // Turn the RED LED off — FUNCTION_Select(FUNCTION_TRANSMIT) turned it on
    // at TX start (functions.c). Without this the LED would remain lit after
    // the CW message finishes transmitting.
...
static void CW_TxStateMachine(void)
{
    // TOT enforcement: when the TX timeout expires the main app (app.c,
    // HandleFunction) sees gTxTimeoutReached while we are in
    // FUNCTION_TRANSMIT, calls APP_EndTransmission() - which de-keys the PA
    // and sets gFlagEndTransmission - and clears gTxTimeoutReached. Without
    // this check the element FSM below would re-key the transmitter on the
    // very next element, transmitting past the configured TOT limit. Abort
    // the playback immediately and tear down the TX path.
    if (gTxTimeoutReached || gFlagEndTransmission)
    {
    ...
    if (gCW_TxEndTime > 0 && (int32_t)(gCW_TxEndTime - gGlobalSysTickCounter) > 0) { return; }
    switch (gCW_TxState)
    {
        ...
        case CW_TX_ELEMENT:
        {
            ...
            {
                while (gCW_TxMsgIdx < gCW_TxSnapshotLen)
                {
                    char c = gCW_TxSnapshot[gCW_TxMsgIdx];
                    if (c >= 'a' && c <= 'z') c = (char)(c - 'a' + 'A');
                    if (c == ' ')
                    {
                        const bool trailing = (gCW_TxMsgIdx + 1) >= gCW_TxSnapshotLen;
                        if (!gCW_TxSentAny || gCW_TxPrevWasSpace || trailing) { gCW_TxMsgIdx++; continue; }
                        gCW_TxPrevWasSpace = true;
                        gCW_TxState = CW_TX_WORD_GAP;
                        gCW_TxEndTime = gGlobalSysTickCounter + (gCW_InterWordMs - gCW_InterCharMs) / 10;
                        gCW_TxMsgIdx++;
                        return;
                    }
                ...
                if (gCW_TxMorse == NULL || gCW_TxMorseIdx >= strlen(gCW_TxMorse))
                {
                    ...
                    gCW_TxEndTime = gGlobalSysTickCounter + gCW_InterCharMs / 10;
                    return;
                }
            }
            if (gCW_TxMorse[gCW_TxMorseIdx] == '.') { CW_ToneOn(); gCW_TxEndTime = gGlobalSysTickCounter + gCW_DitMs / 10; }
            else if (gCW_TxMorse[gCW_TxMorseIdx] == '-') { CW_ToneOn(); gCW_TxEndTime = gGlobalSysTickCounter + gCW_DahMs / 10; }
        ...
        case CW_TX_ELEM_GAP:
        {
            ...
            if (gCW_TxMorse[gCW_TxMorseIdx] != '\0')
            {
                ...
                gCW_TxEndTime = gGlobalSysTickCounter + gCW_InterElemMs / 10;
            }
            else
            {
...
void CW_SendMessage(void)
{
    ...
    // Snapshot the message so edits during TX cannot corrupt the output.
    gCW_TxSnapshotLen = (uint8_t)strlen(gCW_Message);
    memcpy(gCW_TxSnapshot, gCW_Message, gCW_TxSnapshotLen + 1);
    ...
    gCW_TxMorse = NULL;
    gCW_TxMorseIdx = 0;
    gCW_TxEndTime = gGlobalSysTickCounter + 5;
...
// CW typed-message TX is triggered by a SHORT PTT press - on RELEASE - and a
// LONG press is ignored, matching the documented behaviour in
// CW_IMPLEMENTATION.md ("Long press of KEY_PTT in CW compose mode is ignored
// to prevent accidental sends; only short press on release sends typed text").
//
// Why "on release" and not "on press":
//  - Sending on press means the PTT release (debounced 30ms later in
//    CheckKeys()) reaches ProcessKey() while gCurrentFunction is still
//    FUNCTION_TRANSMIT, where GENERIC_Key_PTT(false) immediately tears the RF
//    path down - i.e. a normal short PTT tap aborts the message after ~100ms.
//  - Starting the send only after the release has been fully consumed leaves
//    the CW state machine in CW_COMPOSING at release time, so the event is
//    routed to CW_ProcessKeys() without any end-of-transmission side effects.
//
// Because the hardware PTT never produces bKeyHeld events (it is polled
// out-of-band by CheckKeys() and explicitly excluded from the key-hold path),
// short vs long is determined from press/release timing via
// gGlobalSysTickCounter (10ms units), using the same 400ms threshold as the
// rest of the UI (key_repeat_delay_10ms).
static void CW_HandlePttKey(bool bKeyPressed, bool bKeyHeld)
{
    if (bKeyHeld)
        return;   // defensive: no held events are ever generated for PTT

    if (bKeyPressed)
    {
        // Press: remember the timestamp only when it can legitimately start a
        // send. A press while a message is already being transmitted (or with
        // an empty message) must not queue a send for a later release.
        if (gCW_State == CW_COMPOSING)
        {
            gCW_PttPressTick = gGlobalSysTickCounter;
            gCW_PttPressedInCompose = true;
        }
        else
        {
            gCW_PttPressTick = 0;
    ...
    // ---- Release ----
    if (gCW_State == CW_COMPOSING && gCW_PttPressedInCompose && gCW_CursorPos > 0)
    {
        const uint16_t elapsedTicks = (uint16_t)(gGlobalSysTickCounter - gCW_PttPressTick);
        if (elapsedTicks < CW_PTT_LONG_PRESS_10MS)
        {
            // Short press: send the typed message exactly once.
            gCW_PrevKey = 0;
            gCW_PrevLetter = 0;
            CW_SendMessage();
            CW_Render();
...
void CW_ProcessKeys(KEY_Code_t Key, bool bKeyPressed, bool bKeyHeld)
{
    ...
    // While a typed message is transmitting, ignore all editing/menu keys:
    // the TX path plays the gCW_TxSnapshot taken at send time so live edits
    // cannot corrupt the output, but they would make the display disagree
    // with the text actually being sent. EXIT (abort CW mode) and PTT
    // (routed to the press tracker, which ignores presses made during TX)
    // still pass through.
    if (gCW_State == CW_SENDING && gCW_PlaybackActive &&
        Key != KEY_EXIT && Key != KEY_PTT)
        return;
    ...
    switch (Key)
    {
        ...
        {
            // Only process on key press (not release) to avoid double-cycle
            if (!bKeyPressed) break;
            
            uint8_t idx = (uint8_t)(Key - KEY_1);
            uint8_t count = CW_KEY_CHAR_COUNT[idx];
            
            // MOBILE-STYLE T9:
            // - Same key within timeout: CYCLE character at cursor-1 (no advance)
            // - Otherwise: APPEND first char at cursor (advances cursor)
            ...
            else
            {
                gCW_PrevKey = Key;
                gCW_PrevLetter = 0;
                gCW_Message[gCW_CursorPos++] = CW_GetInputChar(idx, 0);
                gCW_Message[gCW_CursorPos] = '\0';
            }
        ...
        case KEY_0:
        {
            ...
            if (gCW_PrevKey == KEY_0 && gCW_CursorPos > 0)
            {
                if (gCW_Message[gCW_CursorPos - 1] == ' ')
                {
                    // Replace space with '0'
                    gCW_Message[gCW_CursorPos - 1] = '0';
                    gCW_PrevLetter = 1;
                }
                else
                {
            ...
            {
                // First press: append space
                CW_AppendChar(' ');
                gCW_PrevKey = KEY_0; gCW_PrevLetter = 0;
            }
            gCW_KeyTick = 1; CW_Render(); break;
        }
        case KEY_UP:
            // Only on press (not release) to avoid double-delete
            if (bKeyPressed) { gCW_PrevKey = 0; gCW_PrevLetter = 0; gCW_KeyTick = 0; CW_DeleteChar(); CW_Render(); }
        break;
        case KEY_DOWN:
            // Only on press (not release) to avoid double-space
            if (bKeyPressed) { gCW_PrevKey = 0; gCW_PrevLetter = 0; gCW_KeyTick = 0; CW_AppendChar(' '); CW_Render(); }
        break;
        ...
        case KEY_PTT:
            CW_HandlePttKey(bKeyPressed, bKeyHeld);
            break;
        default: break;
    }
}
