...
static void CW_FinalizeRxCharacter(void)
{
    ...
    if (gCW_CharSignalCount > 0)
    {
        ...
        // Map to 0-100 confidence: require at least 8 dB SNR, max at 20 dB
        if (avgQuality >= 8)
        {
            confidence = (uint8_t)((avgQuality - 8) * 100 / 12);
            if (confidence > 100) confidence = 100;
        }
    }
    
    // Adaptive confidence threshold: shorter characters need lower threshold
    // because they have fewer signal samples. Scale from 20% (1 element) to 25% (10 elements).
    uint8_t minConfidence = (uint8_t)(20 + (gCW_RxMorseLen * 5) / 10);
    if (minConfidence > 25) minConfidence = 25;
    if (minConfidence < 20) minConfidence = 20;
