...
// One-shot flag: set when a WORD gap has already emitted its separating space,
// cleared when the next mark rises. Prevents the gap counter (which keeps
// incrementing every tick while silence persists) from emitting one extra
// space every 10 ms for the whole duration of an inter-word pause.
static bool     gCW_RxWordGapEmitted = false;
...
static void CW_ClassifyMark(uint16_t markTicks, uint16_t ditTicks)
{
    ...
    if (gCW_RxMorseLen < CW_RX_MORSE_MAX_LEN)
    {
        ...
        // CW_Render() removed from hot path - main loop updates display
    }
...
static void CW_DecodeCharacter(bool addWordSpace)
{
    ...
    // CW_Render() removed from hot path - main loop handles display
}
...
static void CW_UpdateTraceBuffer(bool signalNow, int16_t rssi_dBm)
{
    ...
    else
    {
        ...
        {
            gCW_RxTraceClock = 0;
            uint8_t sigLevel = (rssi_dBm >= -120) ? (uint8_t)(rssi_dBm + 120) : 0;
...
void CW_Decoder_Reset(bool clearDecodedText)
{
    if (clearDecodedText)
    {
        gCW_DecodeCursor = 0;
        gCW_DecodeText[0] = '\0';
        // Keep calibrated noise floor across resets to avoid brief
        // false triggers from ambient noise during startup/recalibration.
    }
    gCW_RxMorseLen = 0;
    gCW_RxMorse[0] = '\0';
    gCW_RxMarkTicks = 0;
...
void CW_Decoder_ProcessTick(int16_t rssi_dBm)
{
    ...
    if (toneNow)
    {
        ...
        // Transition: silence → tone. The GAP handler (below) already emitted the
        // word-boundary space exactly once (guarded by gCW_RxWordGapEmitted), so no
        // space is inserted here. Begin a fresh mark and allow the next word gap
        // to emit its single separating space.
    ...
    if (gCW_RxDecoderState == CW_RX_STATE_GAP)
    {
        ...
        {
            // Word gap reached: emit exactly ONE separating space for this word
            // boundary. The gap counter keeps incrementing every 10 ms while
            // silence persists, so guard the emission with a one-shot flag to
            // stop it flooding the decoded buffer with an extra space per tick.
            if (!gCW_RxWordGapEmitted)
            {
                if (gCW_RxMorseLen > 0)
                    CW_DecodeCharacter(true);   // pending char + trailing space
                else
                    CW_AppendDecodedText(" ");

                gCW_RxWordGapEmitted = true;
            }
            gCW_RxSpaceTicks = wordGapTicks;  // prevent re-trigger / runaway
            // CW_Render() removed from hot path
        }
    }
}
