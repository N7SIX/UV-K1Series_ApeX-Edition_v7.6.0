...
{
    uint16_t rx_words[MDC1200_RX_FIFO_WORD_COUNT] = {0};
    uint16_t frame_words[MDC1200_FIFO_WORD_COUNT] = {0x5555u, 0x5555u}; /* re-insert HW-synced preamble */
    uint8_t op = 0;
    uint8_t arg = 0;
    uint16_t unit_id = 0;
    bool valid = false;
    ...
    /* Extract 26-byte MDC frame from BK4819 FIFO (13 × 16-bit words) */
    for (i = 0u; i < MDC1200_RX_FIFO_WORD_COUNT; ++i)
        rx_words[i] = BK4819_ReadRegister(BK4819_REG_5F);

    /* Re-arm the FSK receiver for the next frame: clear the RX FIFO and
     * latched interrupt flags without disturbing the interrupt mask. */
    BK4819_WriteRegister(BK4819_REG_59, 0x8068);   /* clear RX FIFO */
    ...
    for (i = 0u; i < MDC1200_RX_FIFO_WORD_COUNT; ++i)
        frame_words[2u + i] = rx_words[i];
    if (MDC1200_DecodeFrameWords(frame_words, ARRAY_SIZE(frame_words), &op, &arg, &unit_id, &valid) == MDC1200_ERROR_NONE && valid) {
