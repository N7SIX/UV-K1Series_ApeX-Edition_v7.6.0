    ...
    switch (gCW_TxState)
    {
            ...
            if (gCW_TxMorse == NULL || gCW_TxMorse[gCW_TxMorseIdx] == '\0')
            {
                while (gCW_TxMsgIdx < gCW_CursorPos)
                {
                    ...
                    if (c == ' ')
                    {
                        ...
                        gCW_TxEndTime = gGlobalSysTickCounter + gCW_InterWordMs;
                        gCW_TxMsgIdx++;
                        return;
                    }
                ...
                if (gCW_TxMorse == NULL || gCW_TxMorseIdx >= strlen(gCW_TxMorse))
                {
                    ...
                    gCW_TxEndTime = gGlobalSysTickCounter + gCW_InterCharMs;
                    return;
                }
            }
            if (gCW_TxMorse[gCW_TxMorseIdx] == '.') { CW_ToneOn(); gCW_TxEndTime = gGlobalSysTickCounter + gCW_DitMs; }
            else if (gCW_TxMorse[gCW_TxMorseIdx] == '-') { CW_ToneOn(); gCW_TxEndTime = gGlobalSysTickCounter + gCW_DahMs; }
        ...
        case CW_TX_ELEM_GAP:
        {
            ...
            if (gCW_TxMorse[gCW_TxMorseIdx] != '\0')
            {
                ...
                gCW_TxEndTime = gGlobalSysTickCounter + gCW_InterElemMs;
            }
            else
            {
...
void CW_SendMessage(void)
{
    ...
    gCW_TxMorse = NULL;
    gCW_TxMorseIdx = 0;
    gCW_TxEndTime = gGlobalSysTickCounter + 50;
