...
int BK4819_TransmitMDC1200Frame(const uint8_t *frame, size_t frame_len)
{
     ...
     * - REG_5D (0x1A00): Frame size = 26 bytes (0x1A = 26)
    ...
    struct reg_value RogerMDC1200_Configuration[] = {
        ...
        { BK4819_REG_5D, 0x1A00 },  // 26 bytes frame: 7 preamble + 5 leader + 14 encoded payload
    ...
    /*
     * Single-burst MDC-1200 timing: 26 bytes × 8 bits/byte ÷ 1200 bps = 173.3ms,
     * plus RF ramp and guard time. This is the supported transmission profile.
     */
    SYSTEM_DelayMs(280);
...
{
    /* Legacy compatibility path: the old long-burst setting is normalized to the
     * supported single-burst MDC-1200 frame so EEPROM values remain readable
     * without exposing the fake long mode in the UI. */
