...
static void SendVersion(uint32_t Port)
{
    ...
    Reply.Header.Size = sizeof(Reply.Data);
    {
        ...
        size_t i;
        for (i = 0; i + 1 < sizeof(Reply.Data.Version) && src[i] != '\0'; i++)
            Reply.Data.Version[i] = src[i];
        Reply.Data.Version[i] = '\0';
    }
...
// FIX #2: EEPROM write offset whitelist
// Only allow writes to EEPROM regions that are safe for CHIRP/UVTools2 compatibility
// Blocks writes to calibration data, boot configuration, and TX lock regions
static bool IsEepromWriteAllowed(uint16_t offset)
{
    // Allow writes to VFO configuration area (standard CHIRP programming region)
    if (offset >= 0x0E98 && offset < 0x0EA0) return true;
    
    // Allow writes to settings/configuration area
    if (offset >= 0x0F30 && offset < 0x0F40) return true;
    
    // Allow writes to channel memory area (CHIRP programs channels here)
    if (offset >= 0x0F50 && offset < 0x0FF0) return true;

    // Allow writes to DTMF code area
    if (offset >= 0x0F10 && offset < 0x0F30) return true;

    // Allow writes to FM radio preset area
    if (offset >= 0x0F00 && offset < 0x0F10) return true;

    // Block everything else (calibration data, boot config, TX lock, etc.)
    return false;
}

// write eeprom
static void CMD_051D(uint32_t Port, const uint8_t *pBuffer)
// REPLACE the CMD_051D function entirely
{
    ...
    if (!bIsLocked)
    {
        ...
        for (i = 0; i < (pCmd->Size / 8); i++)
        {
            ...
            // FIX #2: Check EEPROM write whitelist before writing
            // This prevents overwriting calibration data, boot config, TX lock settings
            if (!IsEepromWriteAllowed(Offset))
            {
                // Silently skip writes to protected regions
                continue;
            }
...
bool UART_IsCommandAvailable(uint32_t Port)
{
    ...
    // FIX #1: DMA buffer overflow protection
    // Ensure the total data fits within the 256-byte command buffer
    // The command buffer size minus header (4 bytes) = 252 bytes max payload
    const uint16_t MaxPayloadSize = sizeof(pUART_Command->Buffer) - sizeof(Header_t);
    ...
    if (TailIndex < Index)
    {
        ...
        // Bounds check: ChunkSize + TailIndex must not exceed Buffer size
        if (ChunkSize + TailIndex > sizeof(pUART_Command->Buffer))
        {
            *pReadPointer = DmaLength;
            return false;
        }
    ...
    {
        const uint16_t CopySize = TailIndex - Index;
        // Bounds check
        if (CopySize > sizeof(pUART_Command->Buffer))
        {
            *pReadPointer = DmaLength;
            return false;
        }
        memcpy(pUART_Command->Buffer, ReadBuf + Index, CopySize);
    }
