...
// Draw timing-diagram style signal graph on CW_LINE_DECODE (5th row)
// Professional CW decoders use a simple on/off pulse train:
//   - Solid top half when CW tone is detected
//   - Blank when no tone
// This clearly shows dit/dah timing, character gaps, and word gaps
static void CW_DrawSignalGraph(void)
{
    // Clear the line first
    memset(gFrameBuffer[CW_LINE_DECODE], 0, LCD_WIDTH);
    
    // Draw timing diagram from history buffer
    // Index 0 = OLDEST (left), index 127 = NEWEST (right)
    for (uint8_t i = 0; i < LCD_WIDTH; i++)
    {
        if (gCW_RxSignalHistory[i] > CW_SIGNAL_THRESHOLD)
        {
            // Draw solid bar in top half (rows 0-3) for signal ON
            gFrameBuffer[CW_LINE_DECODE][i] |= 0x0F;  // Bits 0-3 set
        }
        // No signal: leave blank (cleaner display)
    }
}
...
void CW_TimeSlice10ms(void)
{
    ...
    // Update signal waveform history for scrolling display
    // Each tick shifts the history left and inserts the new sample at the right
    {
        ...
        // Shift history left by one, insert new sample at rightmost position
        memmove(gCW_RxSignalHistory, gCW_RxSignalHistory + 1, 127);
        gCW_RxSignalHistory[127] = sigLevel;
    }
    ...
    if (gCW_RxSignalPrev)
    {
        ...
        {
            // Fuzzy classification with hysteresis to handle timing variation
            const uint16_t lowerThreshold = (uint16_t)((ditTicks * 4) / 2);  // 2.0 * dit
            const uint16_t upperThreshold = (uint16_t)((ditTicks * 6) / 2);  // 3.0 * dit
            bool isDah;
            
            if (gCW_RxMorseLen == 0)
            {
                // First element in character: use strict threshold
                isDah = (gCW_RxMarkTicks >= dahThreshold);
            }
            else
            {
                // Subsequent elements: use fuzzy band
                if (gCW_RxMarkTicks <= lowerThreshold)
                    isDah = false;
                else if (gCW_RxMarkTicks >= upperThreshold)
                    isDah = true;
                else
                {
                    // Gray zone: use ratio of previous dit estimate
                    const uint16_t ratio = gCW_RxMarkTicks / (ditTicks > 0 ? ditTicks : 1);
                    isDah = (ratio >= 3);
                }
            }
...
void CW_Render(void)
{
    ...
    switch (gCW_State)
    {
        case CW_COMPOSING:
        {
            ...
            const bool receiving = gCW_RxActive;

            if (receiving && gCW_DecodeText[0] != '\0')
            {
                const uint16_t len = (uint16_t)strlen(gCW_DecodeText);
                memset(gFrameBuffer[CW_LINE_TX1], 0, LCD_WIDTH);
                memset(gFrameBuffer[CW_LINE_TX2], 0, LCD_WIDTH);
                ...
                if (len <= CW_CHARS_PER_TX_LINE)
                {
                    ...
                    UI_PrintStringSmallNormal(gCW_DecodeText, 28, 0, CW_LINE_TX1);
                }
                else
                {
                    UI_PrintStringSmallNormal("RX:", 2, 0, CW_LINE_TX1);
                    char line1[CW_CHARS_PER_TX_LINE + 1];
                    memcpy(line1, gCW_DecodeText, CW_CHARS_PER_TX_LINE);
                    ...
                    if (remaining > CW_CHARS_PER_TX_LINE)
                        remaining = CW_CHARS_PER_TX_LINE;
                    char line2[CW_CHARS_PER_TX_LINE + 1];
                    memcpy(line2, gCW_DecodeText + CW_CHARS_PER_TX_LINE, remaining);
                    line2[remaining] = '\0';
