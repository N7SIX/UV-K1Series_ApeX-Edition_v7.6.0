...
void APP_TimeSlice10ms(void)
{
    ...
    if (gUART_LockK5Viewer == 0 &&
        ...
        // Rate limit: only send every 100 ticks (1 second)
        if (k5viewerRateLimit_10ms > 0) {
            ...
            if (sig != prevSig) {
                ...
                prevSig = sig;
                k5viewerRateLimit_10ms = 100; // 1 second cooldown
            }
        }
    }
#endif
    ...
    {   // transmitting
        ...
        if (!CW_IsActive() && gSetting_mic_bar && (gFlashLightBlinkCounter % (150 / 10)) == 0) // once every 150ms
            UI_DisplayAudioBar();
#endif
    }
