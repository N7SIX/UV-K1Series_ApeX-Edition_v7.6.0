...
static const uint8_t BITMAP_QR_GitHub_Compressed[137] = {
    ...
    0x2C, 0xE1, 0x34, 0xF8, 0x2C, 0x5E, 0x8B, 0xF8, 0x93, 0xE1, 0xB6,
    0x65, 0x8D, 0x4A, 0x00, 0x7F, 0x41, 0x5D, 0x5D, 0x5D, 0x41, 0x7F,
    0x5B, 0x1F, 0xBA, 0xEE, 0xD9, 0x9F, 0x55, 0x98, 0x74, 0xE2, 0x1A,
    0x0D, 0x96, 0x3D, 0xA5, 0x0D, 0x69, 0xC2, 0xA5, 0xF2, 0x96, 0xF9,
    0x59, 0x5F, 0xDC, 0x4A, 0x14, 0x63, 0x18, 0xD9, 0x47, 0x55, 0x11,
    0x2C, 0x02, 0xD8, 0x64, 0x67, 0x75, 0x55, 0x94, 0x53, 0x7F, 0x45,
    0x0D, 0x96, 0x3D, 0xA5, 0x0D, 0x69, 0xC2, 0xA5, 0xF2, 0x96, 0xDF,
    0x19, 0xF7, 0x5F, 0xC2, 0x15, 0xBC, 0x55, 0x2A, 0x44, 0x44, 0x11,
    0xFD, 0x05, 0x75, 0x75, 0x75, 0x05, 0xFD, 0x01, 0x9A, 0x38, 0x37,
    0x0D, 0x96, 0x3D, 0xA5, 0x0D, 0x69, 0xC2, 0xA5, 0xF2, 0x96, 0xCB,
    0xC0, 0x81, 0xDF, 0x11, 0x75, 0x11, 0x7F, 0x3E, 0x64, 0x37, 0x18,
    0x7F, 0xEB, 0x15, 0x23, 0x01
};
...
{
    // RAM: span from _sdata to _end covers .data + .noncacheable + .bss + the
    // ALIGN(8) padding before the heap/stack reservation.  Then add heap+stack.
    // _end is PROVIDEd in the linker script at the start of ._user_heap_stack.
    const uint32_t heap_size  = (uint32_t)(uintptr_t)&_Min_Heap_Size;
    const uint32_t stack_size = (uint32_t)(uintptr_t)&_Min_Stack_Size;
    *ram_used = span(&_sdata, &_end) + heap_size + stack_size;
