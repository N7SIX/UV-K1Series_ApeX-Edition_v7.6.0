...
## Post-v7.6.10C — Keyboard Responsiveness & Auto-Keypad-Lock Fixes

> **Status:** FIXED

### Problem
Users reported multiple keyboard-related issues:
1. **UV Studio keyboard not controlling the radio** — Serial key injection from K5Viewer/UV Studio was written to `gKeyFromSerial` but never consumed by `KEYBOARD_Poll()`.
2. **Button lag / multiple presses needed** — Keys injected via serial were consumed and cleared in a single poll, preventing the debounce logic (which requires 20ms / 2 polls) from registering the keypress.
3. **Cyclical stuttering on UP/DOWN hold** — Key repeat counter overflow reset misaligned with the modulo-8 repeat boundary, causing timing jitter.
4. **Auto-keypad-lock activating faster than specified** — `gKeyLockCountdown` was not reset when the radio woke from power-save for RX monitoring, causing premature lock during passive reception.

### Root Causes & Fixes

#### 1. Serial Key Injection Not Consumed (keyboard.c)
- **Root cause:** `gKeyFromSerial` was written by `KEYBOARD_InjectKey()` but never read by `KEYBOARD_Poll()`.
- **Fix:** Added consumption logic at the top of `KEYBOARD_Poll()` that returns the serial-injected key with priority over physical keys.
- **File:** `App/driver/keyboard.c` — `KEYBOARD_Poll()`

#### 2. Debounce Race Condition (keyboard.c, app.c)
- **Root cause:** `gKeyFromSerial` was cleared immediately after reading in `KEYBOARD_Poll()`, so the debounce logic in `CheckKeys()` never saw the key for the required 20ms.
- **Fix:** `KEYBOARD_Poll()` now returns the key WITHOUT clearing `gKeyFromSerial`. Added `KEYBOARD_ConsumeSerialKey()` function that `CheckKeys()` calls after the key is processed (after debounce).
- **Files:** `App/driver/keyboard.c`, `App/driver/keyboard.h`, `App/app/app.c`

#### 3. Dead Code Removal (keyboard.c)
- Removed unused `SERIAL_KEY_SHORT_POLLS`, `SERIAL_KEY_LONG_POLLS` macros, `gSerialKeyHoldCount`, and `gSerialKeyLong` variables that were remnants of an incomplete implementation.

#### 4. Key Repeat Counter Alignment (app.c)
- **Root cause:** On `gDebounceCounter` overflow, reset to `key_repeat_delay_10ms + 1` (41) misaligned with the modulo-8 repeat boundary.
- **Fix:** Reset to `key_repeat_delay_10ms + key_repeat_10ms - 1` (47) so the next increment lands on 48 (divisible by 8), maintaining consistent 80ms repeat intervals.
- **File:** `App/app/app.c` — `CheckKeys()`

#### 5. Auto-Keypad-Lock Reset on Wake (app.c)
- **Root cause:** `gKeyLockCountdown` was only reset on key press, not on wake from power-save for RX monitoring. This caused the countdown to expire during passive reception, locking the keypad prematurely.
- **Fix:** Added `gKeyLockCountdown` reset to the power-save wake path in `APP_TimeSlice10ms()`, so every wake event resets the timer.
- **File:** `App/app/app.c` — `APP_TimeSlice10ms()`

#### 6. Documentation Improvements (keyboard.c)
- Added clarifying comments for `PIN_COL` macro (inverted pin mapping matches PCB routing).
- Added comment explaining column scan logic (j=0 = side keys, j=1..4 = matrix).

### Files Changed
- `App/driver/keyboard.c` — Serial key consumption, `KEYBOARD_ConsumeSerialKey()`, dead code removal, documentation
- `App/driver/keyboard.h` — Added `KEYBOARD_ConsumeSerialKey()` declaration
- `App/app/app.c` — Debounce counter alignment, auto-keypad-lock reset on wake, serial key consumption call

### Verification
- Serial key injection from UV Studio/K5Viewer now works correctly
- Debounce logic properly processes serial-injected keys
- Key repeat intervals are consistent (80ms after initial 400ms delay)
- Auto-keypad-lock countdown resets on all wake events
- UPCode/DWCode digit entry now works correctly from MENU

## Post-v7.6.10C — UPCODE/DWCODE Digit Entry Fix

> **Status:** FIXED

### Problem
Pressing MENU on the UPCode or DWCode menu items did not allow digit entry. The `MENU_Key_MENU()` handler returned early without entering sub-menu mode for these menu items, so the digit input handler in `MENU_Key_0_to_9()` was never reached for code editing.

### Root Cause & Fix
- **Root cause:** Lines 1906-1912 in `App/app/menu.c` had an early `return` for `MENU_UPCODE`, `MENU_DWCODE`, and `MENU_ANI_ID` that prevented `gIsInSubMenu` from being set to `true`.
- **Fix:** Removed the early return block. UPCODE/DWCODE now enter sub-menu mode like other editable menu items.
- **Added DTMF code editor:** New `MENU_Key_DTMFCode()` intercepts key input while the UPCode/DWCode sub-menu is active:
  - `KEY_0..KEY_9` → digit `'0'..'9'` appended, cursor advances
  - `KEY_UP` / `KEY_DOWN` (the `<` / `>` selectors) → cycle the alphabet character `'A'..'D'` at the current position; first press places `'A'`, further presses cycle (A→B→C→D→A). E/F are excluded: `DTMF_ValidateCodes()` rejects them (code would reset to default on reboot) and the BK4819 has no DTMF tone pair for them.
  - `KEY_STAR` → `'*'`, `KEY_F` → `'#'`
  - `KEY_MENU` → saves the code and exits the sub-menu (standard accept path, via `MENU_AcceptSetting()`)
  - `KEY_EXIT` → backspace; at cursor position 0 it leaves the sub-menu and persists the code if modified (`gDTMFCodeDirty` guard prevents needless flash writes)
- **Auto-commit at max length:** When the code reaches 15 characters (safe max = array `[16]` minus the NUL terminator), it auto-saves immediately.
- **Cursor starts at end of existing code:** Entering the editor places the cursor after the current code so typing appends; backspace trims from the end.
- **Added save trigger:** Added `MENU_UPCODE`/`MENU_DWCODE` case to `MENU_AcceptSetting()` to trigger EEPROM save on exit.

### Files Changed
- `App/app/menu.c` — Removed early return, added one-tap DTMF editor + key interception, EXIT backspace/persist, dirty flag, sub-menu cursor placement, save trigger

---

## Post-v7.6.10C — MDC-ID Hex Letter Entry via `<` / `>`

> **Status:** FIXED

### Problem
The MDC-ID menu only accepted digits 0-9 from the keypad. Although the value is accumulated as a 4-nibble hex number and the UI renderer (`ui/menu.c` `MENU_MDC_ID` case) already prints nibble values 10-15 as hex letters A-F, there was **no way to enter the letters A-F** from the keypad. Inside the sub-menu, `KEY_UP`/`KEY_DOWN` merely adjusted the stored value ±1.

### Fix
- **New `MENU_Key_MDC_ID_Letter()`** (`App/app/menu.c`): when the MDC-ID sub-menu is active, `KEY_UP`/`KEY_DOWN` (the `<` / `>` selectors) now select hex letters:
  - First press places `'A'` (nibble value 10) at the current input position
  - Further presses cycle that position `A→B→C→D→E→F→A` (UP) or `A→F→E→…` (DOWN)
  - `gInputBox` stores nibble values 10-15 for A-F, which the existing renderer prints as hex letters — no UI change needed
- **Auto-commit:** like the digit path, the 4-nibble value commits to `gEeprom.MDC_UnitID` (with immediate EEPROM save) as soon as the 4th nibble lands
- **Mixed entry:** digits 0-9 (via `INPUTBOX_Append()`) and letters A-F (via UP/DOWN) can be freely combined in any of the 4 positions
- **State resets:** `mdc_letter_cycle` flag reset on sub-menu entry, on EXIT backspace, and after any digit key press, so UP/DOWN always targets the correct position
- **Note:** UP/DOWN no longer adjust the MDC-ID value ±1 while inside the sub-menu editor (that was the only way to reach values with hex letters); menu navigation outside the sub-menu is unchanged

### Files Changed
- `App/app/menu.c` — Added `mdc_letter_cycle` flag, `MENU_Key_MDC_ID_Letter()` handler, UP/DOWN interception for `MENU_MDC_ID`, flag resets (entry/backspace/digit)

---

## Post-v7.6.10C — MDC-1200 RX Framing Fix & Motorola XOR Differential Encoding (5-Pass Audit)

> **Status:** FIXED (all five audit passes complete)

### Investigation (5 Audit Passes)
1. **First pass:** Found 5 root causes (RC1-RC5) in BK4829 framing (REG_5D RX byte count, TX preamble duplication, RX-FIFO clear strobe bug, REG_5C mismatch)
2. **Second pass:** Discovered TX/RX/handler three-way mismatch after initial fix; switched to full-frame TX
3. **Third pass:** Fixed 5 documentation bugs (stale comments, wrong frame sizes in function docs)
4. **Fourth pass:** Added 13 simulation tests (round-trip, ECC, sliding-window sync, preamble verification)
5. **Fifth pass:** Implemented Motorola XOR differential encoding (physical-layer requirement per Batlabs/US Patents 4,457,005 / 4,517,561 / 4,590,473 / 4,517,669)

### Files Changed
- `App/mdc1200.h` — RX FIFO word counts (13/23), TX FIFO lengths (26/46), added DiffEncode/DiffDecode declarations
- `App/mdc1200.c` — Added `MDC1200_DiffEncodeFrame()` and `MDC1200_DiffDecodeFrame()` (Motorola XOR differential encoding)
- `App/driver/bk4829.c` — Full-frame TX (REG_5D=0x1A/0x2E), RX REG_5D=0x1A/0x28, TX encodes local copy before FIFO load
- `App/app/app.c` — Handler reads FIFO directly, RX decodes bytes before decoder, no preamble re-insertion
- `App/radio.c` — Passes frame-mode flag to RX arming call
- `tests/test_mdc1200.c` — 18 total tests (13 simulation + 5 differential encoding), all pass
- `documentation/MDC1200_RX_FRAMING_FIX_v7.6.10C.md` — Full audit record covering all five passes

### Verification
- Host unit tests: **17,556 checks, 0 failures**
- Firmware build: `n7six.ApeX-k1.v7.6.10C` — **FLASH 114,468 B (94.73%), RAM 14,528 B (88.67%)**
- MDC fuzz test: **0 miscorrections** (beyond dfree/2)

### Motorola Physical-Layer Compliance
- XOR differential encoding: `diff[n] = data[n] XOR data[n-1]` applied to entire frame as continuous bit stream
- 0x55 preamble encodes to ~constant tone (0x7F/0xFF) — verified
- NOT using BK4819 "scramble" (REG_31) — correctly avoided (that's a different mechanism)

### On-Air Testing Notes
1. **Receiving channel must have PTT ID set to MDC1200/MDC1200L** (RX is gated on the RX VFO's PTT-ID mode by design)
2. Test with genuine Motorola radio for full interop validation

---

## Post-v7.6.10C — Dead Code Removal & Menu Categorization

- **Files:** `App/app/breakout.c`, `App/app/breakout.h` (deleted), `App/ui/helper.c`, `App/ui/helper.h`, `App/ui/menu.c`, `App/ui/menu.h`
- **Dead code removed (cppcheck + manual verification):** the never-compiled Breakout game (`app/breakout.c/.h` — absent from CMakeLists and never `#include`'d) and its two now-orphaned UI helpers `UI_DrawRectangleBuffer()` / `UI_PrintStringSmallBufferBold()` (declarations and definitions removed from `ui/helper.c/.h`). No remaining references in first-party code.
- **Menu categorized:** `MenuList[]` reordered into functional groups — BASIC, TONE, TX, CHANNEL, RX, SCAN, DTMF, DISPLAY, SYSTEM, KEYS, then the N7SIX `SET_*` block. An on-screen category header was prototyped and removed after review; only the ordering remains.
- **Hidden-menu boundary preserved (critical):** the hidden menu is implemented in `main.c` by truncating the list at the first `MENU_F_LOCK` entry. During the reordering `Reset` was accidentally moved above `F_LOCK` (becoming visible) and the `SET_*` block below it (becoming hidden). Both fixed: the hidden tail restores the original order — `F Lock → Tx200/Tx350/Tx500 → 350 En → ScraEn → FrCali → BatCal → BatTyp → SetNav → Reset (last)` — keeping `Reset` inaccessible without the PTT + side-button power-on combo. Visible item set and count (72) verified identical to the pre-reorder firmware via preprocessed-output diff.
- **SysInf relocated:** `SysInf` (N7SIX builds) moved from DISPLAY to the **last visible slot (position 72)**, directly before the hidden boundary; non-N7SIX builds keep `BatVol` in DISPLAY as before.
- **Verification:** `arm-none-eabi-gcc -fsyntax-only` with the full ApeX/N7SIX define set — EXIT=0; visible count 72, first hidden item `F Lock`, `Reset` hidden (position 78).

## Post-v7.6.10C — MDC-1200 PTT-ID Silence Root-Cause Fix (call order + sync word)

- **Files:** `App/radio.c`, `App/driver/bk4819.c`, `App/driver/bk4829.c`
- **Symptom:** with MDC-1200 / MDC-1200L selected as the channel PTT-ID, the transmitted burst was silent on RX (no preamble sound, nothing decodable) even after restoring the full-frame TX FIFO.
- **Root cause 1 — call-order inversion (primary):** in the audible Roger-menu era the MDC burst fired from `BK4819_PlayRoger()` *before* `DTMF_SendEndOfTransmission()`, i.e. while the chip was still in normal unmuted voice-TX state (`REG_50 = 0x3B18`). The PTT-ID relocation moved the burst *after* `DTMF_SendEndOfTransmission()`, whose `BK4819_ExitDTMF_TX(true)` leaves the chip in `BK4819_EnterTxMute()` (`REG_50 = 0xBB18`). The DTMF tone-oscillator path still modulates while muted, but the FSK data path does not — the burst modulated nothing. **Fix:** the burst now fires first in `RADIO_SendEndOfTransmission()`, restoring the audible-era chip state (ordering comment pinned in the source).
- **Root cause 2 — unvalidated sync-word change (secondary):** `REG_5B` (HW sync) was changed `0x55AA → 0x5555` in the "phase-reversal fix" *after* the last on-air-audible build (archive `6a8acd45`). **Fix:** reverted to the on-air-proven `0x55AA` in `BK4819_TransmitMDC1200Frame()` (both drivers), with a do-not-change-without-on-air-validation comment. The RX sync detector programs the MDC leader (`REG_5A/5B = 0x0709/0x2A44`) and is unaffected.
- **Result:** full firmware builds & links cleanly (`ApeX` option set, native `arm-none-eabi-gcc`, EXIT=0, 90/90 objects); FLASH 114,308 B (94.60 %), RAM 14,528 B. Host suite: 17,277 checks, 0 failures.

## Post-v7.6.10C — MDC-1200 PTT-ID TX Burst Restore ("no preamble sound" fix)

- **Files:** `App/driver/bk4819.c`, `App/driver/bk4829.c`, `App/mdc1200.c`, `App/mdc1200.h`, `tests/test_mdc1200.c`, `documentation/MDC1200_MENU.md`
- **Symptom:** with MDC-1200 / MDC-1200L selected as the channel PTT-ID, the transmitted burst carried no audible preamble/data (nothing decodable, no burst sound on a monitor receiver).
- **Root cause:** the earlier "double-preamble fix" (preamble-stripped TX FIFO, `REG_5D` 0x1400/0x2800, `REG_5B` 0x5555 hardware sync only) did not survive on-air validation — the hardware-generated preamble+sync alone did not produce the expected MDC-1200 burst for genuine decoders. The last on-air-proven implementation (the one that lived behind the Roger menu) always loaded the **full frame, preamble included**, into the TX FIFO.
- **TX restored:** `BK4819_TransmitMDC1200Frame()` (both drivers) is back to the proven framing — full-frame FIFO via `MDC1200_BuildFifoWords()` (23 words / 46 B for the long frame), `REG_5D` = 0x1A00 (26 B) / 0x2E00 (46 B), sync/phase patterns untouched (`REG_5A/5B` = `0x5555`), TX hold 280 ms (standard) / 400 ms (long).
- **Dead code removed:** `MDC1200_BuildTxFifoWords()` (preamble-stripping helper) deleted from `App/mdc1200.c/.h` together with its unit tests; the MDC-1200L-vs-MDC-1200L length-distinction tests now pin `MDC1200_BuildFifoWords()` output (23 vs 13 words).
- **Result:** full firmware builds & links cleanly (native `arm-none-eabi-gcc` 14.3.1 verify build, EXIT=0, 90/90 objects); FLASH 114,308 B (94.60 %), RAM 14,528 B. Host suite: 17,277 checks, 0 failures.
...
> **Superseded (see the "MDC-1200 PTT-ID TX Burst Restore" entry above):** the
> phase-reversal fix below stands, but the "double-preamble fix" it introduced
> (preamble-stripped TX FIFO, `REG_5D` 0x1400/0x2800) was **reverted** after
> on-air testing showed no audible/decodable burst. The full-frame FIFO
> (`MDC1200_BuildFifoWords()`, `REG_5D` 0x1A00/0x2E00) is the proven framing.
