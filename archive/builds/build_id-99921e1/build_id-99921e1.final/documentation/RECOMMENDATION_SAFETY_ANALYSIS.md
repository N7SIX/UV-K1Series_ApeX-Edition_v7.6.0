...
The following recommendations were reviewed for EEPROM/calibration/logic safety and implemented:

| Recommendation | Status | Notes |
|---------------|--------|-------|
| Document SPI timing | ✅ Already documented | No NOP loops remained - timing verified at 48 MHz |
| ISR safety invariant comment | ✅ **IMPLEMENTED** | Added to `st7565.h:27-32` and `waterfall.h:33-36` |
| `#define` for magic SPI addr | ✅ Already existed | `SPECTRUM_SETTINGS_SPI_ADDR` in `spectrum.h:27` |
| gScanRangeStart/Stop validation | ⏳ Deferred | Requires spectrum.c scan-range test |
...
### Additional Fixes Applied (Not in original recommendation list)

1. **K5Viewer rate limiting** (`app.c:1650-1680`) - Fixed key stutter by limiting to 1Hz
2. **Tail tone reduction** (`radio.c:1331`) - Reduced 200ms → 100ms (industry standard)
3. **Waterfall NULL deref fix** (`waterfall.c:68-71`) - Added `gRxVfo` check
4. **Waterfall OOB fix** (`waterfall.c:197`) - Added `peakIndex` bounds check
...
**Update (2026-08-01):** 3 recommendations implemented, 6 deferred (safe to implement later), 1 not recommended.
