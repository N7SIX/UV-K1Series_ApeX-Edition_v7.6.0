...
void CW_AppUpdate(void)
{
    ...
    // Guard: typed-message playback owns the TX path when active.
    // CW_TimeSlice10ms() enforces mutual exclusion, but this guard prevents
    // corruption if CW_AppUpdate() is ever called from outside the main tick.
