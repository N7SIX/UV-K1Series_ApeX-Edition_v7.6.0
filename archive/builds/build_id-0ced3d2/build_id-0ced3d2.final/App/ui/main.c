...
#if defined(ENABLE_FEAT_N7SIX_SCAN_FASTER) && defined(ENABLE_FEAT_N7SIX_SCAN_RSSI)
...
// Smooth audio level with symmetric rise/fall rates
static uint8_t SmoothAudioLevel(uint8_t target, uint8_t current)
{
    // Rise quickly (2 bars per frame), fall slowly (1 bar per frame)
    const uint8_t rise_rate = 2;
    const uint8_t fall_rate = 1;
    
    if (target > current) {
        return current + MIN(target - current, rise_rate);
    } else if (target < current) {
        return current - MIN(current - target, fall_rate);
    }
    return current;
}
#endif
...
void UI_DisplayAudioBar(void)
{
    ...
    static bool     wasPttPressed = false;
    static uint8_t  txWarmupFrames = 0;
    
    // Reset bar level when PTT is first pressed to avoid initial peak spike
    if (gPttIsPressed && !wasPttPressed) {
        barsOld = 0;
        txWarmupFrames = 10;   // skip first 10 frames (~500ms at 50ms/frame)
        wasPttPressed = true;
    ...
    // Skip first frames after PTT so the BK4819 audio DSP path
    // (mic preamp, modulator, CTCSS/DCS injection, TX link) can settle.
    // Reading REG_64 (VoiceAmplitudeOut) too early returns transient spikes.
    if (gCurrentFunction == FUNCTION_TRANSMIT) {
        if (txWarmupFrames > 0) {
            txWarmupFrames--;
            return;
...
#endif  // ENABLE_FEAT_N7SIX_AUDIO_SCOPE
...
void DisplayRSSIBar(const bool now)
{
    ...
    */
    ...
    if(RxLine >= 0 && center_line != CENTER_LINE_IN_USE)
    {
        ...
        // Toggle every other 500ms call (1000ms blink rate) to reduce visual distraction
        if (++blinkDivide >= 2) {
            blinkDivide = 0;
            clean = !clean;
        }
    ...
    // IARU Region 1 R.1: S1..S9 in 6 dB steps, S9 = -93 dBm, S0 below -141 dBm
    if (rssi_dBm >= -93)
        s_level = 9;
...
void UI_DisplayMain(void)
{
    char               String[22];
#ifdef ENABLE_FEAT_N7SIX_CW
    const bool         cwSingleVfoView = CW_IsActive();
#else
...
#ifdef ENABLE_FEAT_N7SIX_CW
    if (cwSingleVfoView && activeTxVFO != vfo_num)
    {
        continue;
    }
#endif
...
#if defined(ENABLE_FEAT_N7SIX_SCAN_SUBAUDIBLE) && ENABLE_FEAT_N7SIX_SCAN_SUBAUDIBLE
        ...
        const bool cwDisplay = CW_IsActive();
...
#ifdef ENABLE_FEAT_N7SIX
    const char *code_list[] = {"", "TTN", "DCR", "DCS"};
#else
    const char *code_list[] = {"", "CT", "DCR", "DCS"};
#endif

    if (code_type < ARRAY_SIZE(code_list))
        s = code_list[code_type];

    // Replace CTCSS label based on state (TX vs RX)
    if (code_type == 1) { // CTCSS
        const bool tx_has_ctcss = (vfoInfo->pTX->CodeType == 1);
        const bool rx_has_ctcss = (vfoInfo->pRX->CodeType == 1);

        // Check if the radio is currently in Transmit mode
        const bool is_transmitting = (gCurrentFunction == FUNCTION_TRANSMIT);

        if (is_transmitting) {
            // When actively transmitting, show TTN if TX tone is active
            if (tx_has_ctcss) {
                s = "TTN";
            }
        } else {
            ...
            if (tx_has_ctcss && rx_has_ctcss) {
                s = "TSQ"; // Both TX and RX CTCSS set
        ...
        if (gSetting_set_gui)
        {
            UI_PrintStringSmallNormal(s, LCD_WIDTH + 52, 0, line + 1);
        ...
        {
            if ((s != NULL) && (s[0] != '\0')) {
                GUI_DisplaySmallest(s, 52, line == 0 ? 17 : 49, false, true);
            }
...
#if ENABLE_FEAT_N7SIX
        {
            if (gSetting_set_gui)
            {
                UI_PrintStringSmallNormal("R", LCD_WIDTH + 65, 0, line + 1);
            }
            else
            {
                GUI_DisplaySmallest("R", 48, line == 0 ? 17 : 49, false, true);
            }
        }
#else
            UI_PrintStringSmallNormal("R", LCD_WIDTH + 59, 0, line + 1);
#endif
