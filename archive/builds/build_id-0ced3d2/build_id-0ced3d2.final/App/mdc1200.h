...
#define MDC1200_RX_FIFO_WORD_COUNT ((MDC1200_FRAME_LENGTH - MDC1200_RX_SYNC_BYTES) / 2u)
 ...
 * 
 * Frame structure: 27-byte composite preamble (20-byte extended pretime +
 * 7-byte standard bit sync, all 0x55) + 5-byte leader + 14-byte encoded
 * payload = 46 bytes
 *
 ...
 * @param frame         - Output buffer for 46-byte frame
 * @param frame_size    - Size of output buffer (must be >= 46)
 * @param frame_len_out - Output: actual frame length (always 46 on success)
 *
 ...
 *
 * @param frame               - 46-byte MDC frame (from MDC1200_BuildFrame)
 * @param frame_len           - Frame length (must be 46)
 * @param fifo_words          - Output buffer for 16-bit FIFO words (23 words for 46 bytes)
 * @param fifo_word_capacity  - Capacity of fifo_words buffer
 * @param fifo_word_count_out - Output: actual word count (23 for standard frame)
 *
 * @return MDC1200_ERROR_NONE (0) on success, negative MDC1200_Error_t code on failure
 *
 ...
 *
 * @param frame         - Raw 46-byte MDC frame including 27-byte composite preamble.
 * @param frame_len     - Frame length (must be 46)
