...
void SCREENSHOT_Update(bool force)
{
    ...
    if (keepAlive > 0) {
        ...
        if (gGlobalSysTickCounter - lastZeroTime >= 100) { // 100 * 10ms = 1000ms = 1 second
            lastZeroTime = gGlobalSysTickCounter;
            if (--keepAlive == 0) {
    ...
    if (!force) {
        uint32_t limit = 100; // 100ms general rate limiting
#ifdef ENABLE_FEAT_N7SIX_CW
        if (CW_IsActive()) {
            limit = 200; // 200ms limit during active CW decoding/typing to prioritize key responsiveness
        }
#endif
        if (gGlobalSysTickCounter - lastTxTime < limit) {
            return;
        }
    }

    lastTxTime = gGlobalSysTickCounter;
