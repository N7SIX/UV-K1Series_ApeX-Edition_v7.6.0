...
/* NOTE: This driver performs blocking SPI transfers and operates on
 * shared framebuffer memory (`gFrameBuffer`, `gStatusLine`). These
 * routines are NOT safe to call from interrupt context (ISRs). They
 * block on SPI hardware flags and assume caller runs in the main
 * task/context. Calling from an ISR may stall interrupts and
 * desynchronize radio timing — avoid doing that.
 */
...
{
    /* Blocking SPI byte transfer. This waits for TXE/RXNE flags and
     * therefore is not suitable for ISR use. Keep transfers short and
     * call from non-interrupt context. */
...
{
    /* Hardware reset not supported on K1 board: kept as a no-op for
     * compatibility with other platforms. Do not rely on this to
     * perform any timing-sensitive operations. If a hardware reset
     * is required, implement it in board-specific code. */
}
