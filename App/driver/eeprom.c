...
/* =========================================================================
 * NOTE (2026-09-08): this file implements the EEPROM logical-address API on
 * top of the PY25Q16 serial flash. It was formerly named driver/eeprom_compat.c
 * and was merged into this canonical name because the I2C-EEPROM
 * implementation this file previously held is unused on this port (the
 * original is preserved in git history). Consumers such as settings.c,
 * uart.c, aircopy.c, dtmf.c and menu.c only see the EEPROM_* API and the
 * ADDR_MAPPINGS virtual-to-physical mapping.
 * ========================================================================= */
...
static const AddrMapping_t ADDR_MAPPINGS[] = {
    ...
    _MK_MAPPING(0x00A000, 0x00A000, 0x00A170),  // Settings * 16 Bytes (ex 0x004000)        0x00A000 -> 0x00A010
                                                ...
                                                // Settings N7SIX * 8 Bytes (0x00C000)      0x00A158 -> 0x00A160
...
// Write-verify state: set to false if any chunk of the last EEPROM_WriteBuffer()
// failed read-back verification after one retry. Read via EEPROM_LastWriteVerified().
static bool s_lastWriteVerified = true;

// Returns true if the last EEPROM_WriteBuffer() verified successfully.
// With deferred flash writes enabled, the per-chunk verify is done against
// the coherent sector cache; the real flash read-back verify happens when
// the dirty sector is flushed, so both results are combined here.
bool EEPROM_LastWriteVerified(void)
{
    return s_lastWriteVerified && PY25Q16_LastFlushVerified();
}
...
void EEPROM_WriteBuffer(uint16_t Address, const void *pBuffer)
{
    ...
    while (Size)
    {
        ...
        if (PY_Addr < HOLE_ADDR)
        {
            ...
            // Read-back verify with one retry (reads are free, no extra wear
            // unless the first verify fails). Guards against power-loss or
            // flash cell failures leaving corrupted settings behind.
            while (retries--)
            {
                PY25Q16_ReadBuffer(PY_Addr, verifyBuf, PY_Size);
                if (memcmp(verifyBuf, pBuffer, PY_Size) == 0)
                {
                    break;
                }
                if (retries)
