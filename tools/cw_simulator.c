...
static void CW_UpdateTiming(CWSession_t *cw)
{
    ...
    if (cw->dit_ms > 120) cw->dit_ms = 120;
...
static void CW_TimeSlice(CWSession_t *cw, bool signal_present)
{
    ...
    uint16_t dah_threshold = 2 * dit_ticks;
...
int main(void)
{
    ...
    printf("TEST 2: CW_Start() WITH mode switch (APP_StartListening)\n");
    printf("----------------------------------------\n");
    ...
    SimulateSignalPattern(&cw, sos_morse);
    ...
    bool test2_pass = (strcmp(cw.decode_text, "SOS") == 0);
    bool test3_pass = (strcmp(cw.decode_text, "CQCQCQDEN7SIX") == 0);
    ...
    printf("Test 3 (complex msg):       %s (expected: CQCQCQDEN7SIX)\n",
    ...
    if (test2_pass) {
        ...
        printf("\nFIX VERIFIED:\n");
        printf("  - With APP_StartListening(RECEIVE), RSSI works correctly\n");
