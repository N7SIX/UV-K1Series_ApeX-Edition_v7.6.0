"""
UV-K1 Series - N7SIX CHIRP Driver

Copyright (c) 2026 Sean (N7SIX)

Based on original work by Jacek Lipkowski (SQ5BPF), EGZUMER, F4HWN, JOC2, F1ixx, and Dan Smith (template.py)

This driver enables CHIRP support for the Quansheng UV-K1/K5 (ApeX Edition) and compatible radios.

License: GNU General Public License v2 or later (GPL-2.0+)

THIS SOFTWARE IS PROVIDED "AS IS" WITHOUT WARRANTY OF ANY KIND. USE AT YOUR OWN RISK.

Project: https://github.com/N7SIX/uvk1k5-apex-chirp-driver
Contact: Sean (N7SIX)

Changelog and prior authors preserved below.
"""
...
#       No pop-up warnings added; all warnings are logged
#       Codebase now fully English and robust for CHIRP-next
...
class UVK5RadioEgzumer(chirp_common.CloneModeRadio):
    ...
    def get_raw_memory(self, number):
        # N7SIX: Bounds check for channel number
        if not (1 <= number <= len(self._memobj.channel)):
            raise errors.RadioError(f"Channel number {number} out of range (1-{len(self._memobj.channel)})")
    ...
    def get_memory(self, number):
        # N7SIX: Added bounds/type checks and robust field access
        mem = chirp_common.Memory()
        if isinstance(number, str):
            specials = self._get_specials()
            if number not in specials:
                raise errors.RadioError(f"Special channel '{number}' not found.")
            ch_num = specials[number]
            mem.extd_number = number
        else:
            ch_num = number - 1
        if not (0 <= ch_num < len(self._memobj.channel)):
            raise errors.RadioError(f"Channel index {ch_num} out of range (0-{len(self._memobj.channel)-1})")
        _mem = self._memobj.channel[ch_num]
        is_empty = (_mem.freq == 0xffffffff) or (_mem.freq == 0)
        # N7SIX: Defensive attribute access for channel attributes
        tmpscn = 0
        tmp_comp = 0
        if ch_num < len(self._memobj.ch_attr):
            _mem3 = self._memobj.ch_attr[ch_num]
            tmpscn = getattr(_mem3, 'is_scanlistx', 0)
            tmp_comp = list_def(getattr(_mem3, 'compander', 0), COMPANDER_LIST, 0)
        elif ch_num < 214:
            att_num = 200 + int((ch_num - 200) / 2)
            if att_num < len(self._memobj.ch_attr):
        ...
        if is_empty:
            ...
            mem.extra.append(RadioSetting("txLock", "TXLock", RadioSettingValueList(SET_OFF_ON_LIST)))
            mem.extra.append(RadioSetting("busyChLockout", "BusyCL", RadioSettingValueBoolean(False)))
            mem.extra.append(RadioSetting("frev", "FreqRev", RadioSettingValueBoolean(False)))
            mem.extra.append(RadioSetting("pttid", "PTTID", RadioSettingValueList(PTTID_LIST)))
            if getattr(self._memobj.BUILD_OPTIONS, 'ENABLE_DTMF_CALLING', False):
                mem.extra.append(RadioSetting("dtmfdecode", "DTMF decode", RadioSettingValueBoolean(False)))
            mem.extra.append(RadioSetting("scrambler", "Scrambler", RadioSettingValueList(SCRAMBLER_LIST)))
            mem.extra.append(RadioSetting("compander", "Compander", RadioSettingValueList(COMPANDER_LIST)))
            mem.extra.append(RadioSetting("scanlists", "Scanlists", RadioSettingValueList(SCANLIST_LIST)))
            return mem
        if ch_num > 199:
            vfo_names = self._get_vfo_channel_names()
            idx = ch_num-200
            mem.name = vfo_names[idx] if idx < len(vfo_names) else f"VFO{idx}"
            mem.immutable = ["name", "scanlists"]
        else:
            _mem2 = self._memobj.channelname[ch_num]
            mem.name = "".join([c for c in _mem2.name if c not in ("\xFF", "\x00")]).rstrip()
        ...
        tstep = _mem.step
        mem.tuning_step = STEPS[tstep] if tstep < len(STEPS) else 2.5
        # N7SIX: Power level assignment with validation
        txp = _mem.txpower
        if txp == POWER_HIGH:
            mem.power = UVK5_POWER_LEVELS[7]
        elif txp == POWER_MEDIUM:
        ...
        mem.empty = is_empty
        mem.extra = RadioSettingGroup("Extra", "extra")
        enc = list_def(getattr(_mem, 'txLock', 0), SET_OFF_ON_LIST, 0)
        mem.extra.append(RadioSetting("txLock", "TX Lock  (TXLock)", RadioSettingValueList(SET_OFF_ON_LIST, None, enc)))
        mem.extra.append(RadioSetting("busyChLockout", "Busy Ch Lockout    (BusyCL)", RadioSettingValueBoolean(getattr(_mem, 'busyChLockout', False))))
        mem.extra.append(RadioSetting("frev", "Reverse Frequencies (R)", RadioSettingValueBoolean(getattr(_mem, 'freq_reverse', False))))
        pttid = list_def(getattr(_mem, 'dtmf_pttid', 0), PTTID_LIST, 0)
        mem.extra.append(RadioSetting("pttid", "Transmission ID (PTT ID)", RadioSettingValueList(PTTID_LIST, None, pttid)))
        if getattr(self._memobj.BUILD_OPTIONS, 'ENABLE_DTMF_CALLING', False):
            mem.extra.append(RadioSetting("dtmfdecode", "Decodage DTMF (D Decd)", RadioSettingValueBoolean(getattr(_mem, 'dtmf_decode', False))))
        enc = list_def(getattr(_mem, 'scrambler', 0), SCRAMBLER_LIST, 0)
        mem.extra.append(RadioSetting("scrambler", "Scrambler (Scramb)", RadioSettingValueList(SCRAMBLER_LIST, None, enc)))
        mem.extra.append(RadioSetting("compander", "Compander (Compnd)", RadioSettingValueList(COMPANDER_LIST, None, tmp_comp)))
        mem.extra.append(RadioSetting("scanlists", "Scanlists (SList)", RadioSettingValueList(SCANLIST_LIST, None, tmpscn)))
        return mem
        ...
        # PTTID
        pttid = list_def(_mem.dtmf.pttid, PTTID_LIST, 0)
        ...
        # DTMF DECODE
        val = RadioSettingValueBoolean(_mem.dtmf.decode_response)
    ...
    def set_settings(self, settings):
        # N7SIX: Refactored for maintainability, type/bounds checks, and logging
        _mem = self._memobj
        def safe_set(attr, val, minval=None, maxval=None, enum=None, logname=None):
            try:
                if enum:
                    if val not in enum:
                        raise ValueError(f"Value {val} not in allowed set {enum}")
                if minval is not None and val < minval:
                    raise ValueError(f"Value {val} below minimum {minval}")
                if maxval is not None and val > maxval:
                    raise ValueError(f"Value {val} above maximum {maxval}")
                setattr(_mem, attr, val)
                if logname:
                    LOG.debug(f"N7SIX: Set {logname} to {val}")
            except Exception as e:
                LOG.warning(f"N7SIX: Failed to set {attr}: {e}")
        ...
        for element in settings:
            ...
            val = element.value
            # N7SIX: Example for a few fields, expand as needed
            if elname == "VFO_A_chn":
                safe_set("ScreenChannel_A", int(val), 0, 255, logname="ScreenChannel_A")
            elif elname == "VFO_B_chn":
                safe_set("ScreenChannel_B", int(val), 0, 255, logname="ScreenChannel_B")
            elif elname == "TX_VFO":
                safe_set("TX_VFO", int(val), 0, 1, logname="TX_VFO")
            elif elname == "call_channel":
                safe_set("call_channel", int(val), 0, 255, logname="call_channel")
            elif elname == "squelch":
                safe_set("squelch", int(val), 0, 255, logname="squelch")
            elif elname == "tot":
                safe_set("max_talk_time", int(val), 0, 255, logname="max_talk_time")
            elif elname == "noaa_autoscan":
                safe_set("noaa_autoscan", int(val), 0, 1, logname="noaa_autoscan")
            elif elname == "vox":
                voxvalue = int(val)
                safe_set("vox_switch", voxvalue > 0, logname="vox_switch")
                safe_set("vox_level", (voxvalue - 1) if voxvalue > 0 else 0, 0, 10, logname="vox_level")
            elif elname == "mic_gain":
                safe_set("mic_gain", int(val), 0, 255, logname="mic_gain")
            elif elname == "channel_display_mode":
                safe_set("channel_display_mode", int(val), 0, 3, logname="channel_display_mode")
                ...
                if val > 200 or val < 1:
                    ...
                    val -= 1
                _mem[elname] = val
            elif elname == "key1_shortpress_action":
                ...
                try:
                    ...
                    LOG.warning(f"N7SIX: Calibration set failed for {elname}: {e}")
            else:
                try:
                    setattr(_mem, elname, val)
                    LOG.debug(f"N7SIX: Set {elname} to {val}")
                except Exception as e:
                    LOG.warning(f"N7SIX: Could not set {elname}: {e}")
...
# add menu firmware with version and option display if version 3.0 and up
        ...
        if self.FIRMWARE_VERSION == "":
            ValFirm = "Firmware: Only when read from radio"
...
# add link for mise a jour information
        ...
        def validate_Go_Web_Firmware(value):
            if value :
                msg = "Pour voir les informations des mises a jour du Firmware F4HWN \n"
        ...
        rs = RadioSetting("Update_Firmware_mise_a_jour","Pour voir les informations des mises a jour du Firmware F4HWN, selectionner cette boite ->", val)
        rs.set_doc('Pour voir les informations des mises a jour du firmware F4HWN!')
...
# end add menu firmware with version and option display
        ...
        SetTmrSetting.set_doc('SetTmr: Pour Activer ou Desactiver le timer sur une transmission ou une reception (a gauche de la bare de status...)\n' + \
                              '* OFF : Desactiver \n' + \
                              '* ON : Activer ')
        ...
        SetOffSetting.set_doc('SetOff: Mettre la radio en mode battery preservation, lorsqu\'il n\'y a pas de RX ou de touche presser \n' + \
                              'Pour vous informer que le radio est dans ce mode, La Led clignottera en rouge\n' + \
                              'L\'affichage s\'ettein completement et le hardware tombe en dormance')
        ...
        SetNFMSetting.set_doc('SetNFM: Reglage de la largeur de bande Narrow FM \n' + \
                              '* 12.5 kHz \n' + \
                              '* 6.25 kHz')
        ...
        SetKEYSetting.set_doc('SetKEY: Reglage de la touche pour activer le mode RescueOps')
        ...
        SetMenuLockSetting.set_doc('Active ou desactive le mode RescueOps')
        ...
        auto_keypad_lock_setting.set_doc('KeyLck : Active/Desactive le verouillage automatique du clavier. \n' + \
                                         ...
                                         'ou Temps d\'inactivite (de 15s à 10m).')
        ...
        bat_save_setting.set_doc('BatSav : Option d\'economie Batterie. Un ratio est definie entre le temps d\'activiter de la radio ' + \
        ...
        freq_mode_allowed_setting = RadioSetting("freq_mode_allowed", "Frequence mode autoriser", val)
        ...
        scn_rev_setting.set_doc('ScnRev : Option possible\n' + \
                                ...
                                '* CARRIER : Continu a ecouter X secondes apres disparition du signal et reprend le SCAN\n' + \
                                '* TIMEOUT : Reprend le SCAN apres X secondes d\'ecoute.')
...
#       5 youtube channel YOUTUBE_INFO_PY2MAJ_BRAZIL
        ...
        def validate_Go_Web_Page7(value):
            if value :
                msg = "Ouvrir la page Youtube web de PY2MAJ_BRAZIL \n" \
        ...
        rs = RadioSetting("CHAINE_YOUTUBE7","Youtube de PY2MAJ_BRAZIL, selectionner cette boite ->", val)
        rs.set_doc('Pour des videos d\'explication du firmware F4HWN')
        roinfo.append(rs)
...
#       6 youtube channel YOUTUBE_INFO_HA8CSY_HUNGARIA
        ...
        def validate_Go_Web_Page8(value):
            if value :
                msg = "Ouvrir la page Youtube web de HA8CSY_HUNGARIA \n" \
        ...
        rs = RadioSetting("CHAINE_YOUTUBE8","Youtube de HA8CSY_HUNGARIA, selectionner cette boite ->", val)
        rs.set_doc('Pour des videos d\'explication du firmware F4HWN')
        roinfo.append(rs)
...
#       7 youtube channel YOUTUBE_INFO_4S7JL_SRI_LANKA
        ...
        def validate_Go_Web_Page9(value):
            if value :
                msg = "Ouvrir la page Youtube web de 4S7JL_SRI_LANKA \n" \
        ...
        rs = RadioSetting("CHAINE_YOUTUBE9","Youtube de 4S7JL_SRI_LANKA   selectionner cette boite ->", val)
        rs.set_doc('Pour des videos d\'explication du firmware F4HWN')
        roinfo.append(rs)
...
#
        ...
        append_label(help_group,"(0) = Scan memoire qui sont dans aucune list", "Sur la droite de chaque VFO " )
        append_label(help_group,"(1) = Scan memoires de la list 1", "Sur la droite de chaque VFO " )
        append_label(help_group,"(2) = Scan memoires de la list 2", "Sur la droite de chaque VFO " )
        append_label(help_group,"(3) = Scan memoires de la list 3", "Sur la droite de chaque VFO " )
        append_label(help_group,"(1 2 3) = Scan memoires de tout les lists", "Sur la droite de chaque VFO " )
        ...
        # -------- LAYOUT
        append_label(basic,
                     "=" * 6 + " Debut des Parametres de F4HWN " + "=" * 300, "=" * 300)
